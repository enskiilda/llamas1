...
function GenericInvocation({
  ...
  // Extract readable information from args instead of showing raw JSON
  const getActionDescription = () => {
    if (!args) return "Processing...";
    
    // For computer_use actions
    if (args.action) {
      const action = args.action;
      const details: string[] = [];
      
      if (args.coordinate) {
        details.push(`at (${args.coordinate[0]}, ${args.coordinate[1]})`);
      }
      if (args.text) {
        details.push(`"${args.text}"`);
      }
      if (args.key) {
        details.push(`"${args.key}"`);
      }
  ...
  return (
    <div className="rounded-md bg-zinc-50 p-3 text-sm dark:bg-zinc-900">
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <div className="font-mono text-xs uppercase text-zinc-500 mb-1">{toolName}</div>
          <div className="text-sm text-zinc-700 dark:text-zinc-300">{getActionDescription()}</div>
        </div>
        <div className="flex h-5 w-5 items-center justify-center">{statusIcon}</div>
      </div>
    </div>
